 ...
 * Col F (Customer) = person or lone business; Col I (Company) = employer/bill-to company.
 * Persons → kit_customers.company_id → kit_company_customers.company_id.
...
var CFG = {
  ...
  timeZone: '',
  /**
   * Tabs wiped by wipeKitProjectionSheets() — headers kept, data rows cleared.
   * Never includes reference / rate sheets (see wipeProtectedSheetNames).
   */
  wipeSheetNames: [
    'kit_waybills',
    'kit_customers',
...
var SRC_ALIASES = {
  ...
  deliveryId: { required: false, aliases: ['delivery_id', 'delivery id', 'del_id', 'delivery'] },
...
/** Spreadsheet formula failures (#ERROR!, #N/A) are not real source values. */
...
/**
 * Compact person labels in memory only. Never write the Waybills source sheet.
 */
function rewriteWaybillCompoundSurnames_(ss, dtoRows) {
  if (dtoRows) {
    for (var i = 0; i < dtoRows.length; i++) {
      var d = dtoRows[i];
      if (!d.customerName || d.isCompanyCustomer || isLikelyBusinessName_(d.customerName)) continue;
...
/** Prefer col I company; else col F when F itself is a business. */
function resolveCompanyLabelFromDto_(d) {
  if (d.companyName && !isPlaceholderCompanyLabel_(d.companyName)) {
    return d.companyName;
  }
  if (d.isCompanyCustomer || isLikelyBusinessName_(d.customerName)) {
    return cleanText_(d.customerName);
  }
  return '';
}
...
function writeRowErrors_(ss, errors) {
  ...
  var sheet = ss.getSheetByName(CFG.errorSheetName);
  if (!sheet) {
...
function applyUpserts_(sheet, patches, appends, numCols) {
  ...
  if (patches.length) {
    patches = patches.slice().sort(function (a, b) {
  ...
  if (appends.length > 0) {
    ...
    if (start < CFG.dataStartRow) start = CFG.dataStartRow;
 ...
 * Column A = status flags; B = Delivery trip id; C = Dispatch Date.
 */
...
function readWaybillsSource_(ss) {
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    var dispatch = normalizeDispatchDateOnly_(
      valueByField_(raw, sourceIdx, 'dispatchDate') !== '' && valueByField_(raw, sourceIdx, 'dispatchDate') != null
        ? valueByField_(raw, sourceIdx, 'dispatchDate')
        : raw[WAYBILLS_COL_DISPATCH_FALLBACK_]
    );
    ...
    var dto = {
      ...
      cityName: cleanText_(valueByField_(raw, sourceIdx, 'cityName')),
      ...
      cell: cleanText_(valueByField_(raw, sourceIdx, 'cell')),
      telephone: cleanText_(valueByField_(raw, sourceIdx, 'telephone')),
      email: cleanText_(valueByField_(raw, sourceIdx, 'email')),
      address: cleanText_(valueByField_(raw, sourceIdx, 'address')),
    ...
    // Col F person vs business; col I is the linked company (when present).
    dto.isCompanyCustomer = isLikelyBusinessName_(dto.customerName);
  ...
  return { rows: rows, sourceIdx: sourceIdx, headers: headers, errors: errors };
}
...
/**
 * Stage: Deliveries / trucks only — trips from Waybills Delivery + Dispatch Date.
...
/**
 * Stage: Customers — Waybills col F + col I:
 *   person (F)  → kit_customers, optional company_id from col I
 *   company (I or business F) → kit_company_customers
 * Append-only; near-dupe keys aligned with KIT_Customers PHP normalize_* helpers.
 */
...
function mergeCandidateContact_(cand, d, cityMap) {
  if (d.cell) cand.cell = d.cell;
  if (d.telephone) cand.telephone = d.telephone;
  if (d.email) cand.email = d.email;
  if (d.address) cand.address = d.address;
  if (d.customerId) cand.preferredId = d.customerId;
  if (!cand.cityId && (d.cityId || d.cityName)) {
    cand.cityId = d.cityId || lookupCityIdByName_(cityMap, d.cityName) || '';
  }
}
...
/**
 * Persons only from Waybills col F → kit_customers.
 */
function stagePersonCustomers_(ss, dtoRows, cityMap, companyKeyToId) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (isLikelyBusinessName_(label)) continue;
    if (idx.surname >= 0) {
      ...
      if (fixedSurname && fixedSurname !== currentSurname) {
        ...
        patches.push({ sheetRow: i + 1, values: patched });
    ...
    if (key && key.indexOf('p:') === 0) {
      ...
      existingLabels.push({ key: key, label: normalizePersonNameKey_(label, '') });
    }
  }
  ...
  for (var r = 0; r < dtoRows.length; r++) {
    ...
    if (d.isCompanyCustomer || isLikelyBusinessName_(name)) {
    ...
    if (!candidates[ckey]) {
      candidates[ckey] = {
        ...
        cityId: d.cityId || lookupCityIdByName_(cityMap, d.cityName) || '',
        cell: d.cell || '',
      ...
      mergeCandidateContact_(candidates[ckey], d, cityMap);
  ...
  for (var k = 0; k < candKeys.length; k++) {
    ...
    maxId += 1;
    var newCustId = cand.preferredId || Math.max(maxCustId + 1, CFG.customerSeedMinId);
    if (!cand.preferredId) {
      maxCustId = newCustId;
    } else if (newCustId > maxCustId) {
      maxCustId = newCustId;
    }
    var split = splitPersonName_(cand.label);
    var out = emptyRow_(headers.length);
    if (idx.id >= 0) out[idx.id] = maxId;
    ...
    if (idx.company >= 0) out[idx.company] = cand.companyLabel || '';
    ...
    if (idx.cell >= 0) out[idx.cell] = cand.cell || '';
    if (idx.telephone >= 0) out[idx.telephone] = cand.telephone || '';
    if (idx.email >= 0) out[idx.email] = cand.email || '';
    if (idx.address >= 0) out[idx.address] = cand.address || '';
...
/**
 * Companies from Waybills col I (and business labels in col F) → kit_company_customers.
 */
function stageCompanyCustomers_(ss, dtoRows, cityMap) {
  ...
  for (var r = 0; r < dtoRows.length; r++) {
    ...
    // Lone business in F when I empty already covered by resolveCompanyLabelFromDto_.

    if (!labels.length) {
      if (!d.customerName) summary.skipped_no_name++;
      else if (!(d.isCompanyCustomer || isLikelyBusinessName_(d.customerName))) summary.skipped_person++;
      continue;
    }
  ...
  for (var k = 0; k < candKeys.length; k++) {
    ...
    maxId += 1;
    var newCompanyId = cand.preferredId || Math.max(maxCompanyId + 1, CFG.companySeedMinId);
    if (!cand.preferredId) {
      maxCompanyId = newCompanyId;
    } else if (newCompanyId > maxCompanyId) {
      maxCompanyId = newCompanyId;
    }
 ...
 * Does not create deliveries. delivery_id = Waybills Delivery trip id (or blank for warehouse).
...
function stageWaybills_(ss, dtoRows, partyMaps, rowErrors) {
  ...
  for (var r = 0; r < dtoRows.length; r++) {
    ...
    var deliveryId = d.isWarehouse ? '' : (d.deliveryTripId > 0 ? d.deliveryTripId : (link.delivery_id || ''));
    var directionId = d.directionId || link.direction_id || CFG.defaultDirectionId;
    var cityId = d.cityId || link.city_id || lookupCityIdByName_(cityMap, d.cityName) || '';
    var companyLabel = resolveCompanyLabelFromDto_(d);
    var personLabel = (d.isCompanyCustomer || isLikelyBusinessName_(d.customerName))
      ? ''
      : cleanText_(d.customerName);
    var customerId = '';
    var companyId = '';
    if (personLabel) {
      customerId = (!d.isCompanyCustomer && d.customerId)
        ? d.customerId
        : (link.customer_id || lookupCustomerIdByKey_(custKeyToId, personLabel) || '');
    }
    if (companyLabel) {
    ...
    // Fallback: business-only F with no I already sets companyLabel.
    if (!customerId && !companyId && d.customerId) {
      if (d.isCompanyCustomer) companyId = d.customerId;
...
function validateDto_(d, cityMap) {
  ...
  if (!d.isWarehouse && !(d.deliveryTripId > 0)) {
    issues.push({ severity: 'warn', message: 'Missing Delivery trip id' });
  }
...
function wipeKitProjectionSheets() {
  ...
  var confirm = ui.alert(
    ...
    'Clears DATA rows (keeps headers) on:\n\n' + names.join('\n') +
      '\n\nLEFT ALONE:\n' + protectedNames.join('\n') +
      '\n\nContinue?',
...
/**
 * Clear everything below the header row. If the sheet is empty, no-op.
...
function runWaybillsOnly() {
  return withLock_(function () {
    ...
    writeRowErrors_(ss, rowErrors);
    logSummary_({ customers: customers.summary, waybills: result.summary });
    return result.summary;
  });
}
